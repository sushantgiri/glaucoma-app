...
# -------------------------------------------------------------------
# Cached model loading – called only when needed
# -------------------------------------------------------------------
@st.cache_resource(show_spinner=False)
def get_ensemble():
    from backend.inference_core import GlaucomaEnsemble
    return GlaucomaEnsemble()
...
if uploaded_file is not None and st.session_state.run_requested:
    ...
    with tab_summary:
        ...
        with col3:
            agree_count = sum(
                1 for info in per_model.values() if info["pred_label"] == pred_label
            )
            st.markdown(
                f'''
                <div class="gl-metric-card">
        ...
        st.markdown(agreement_bar(pred_label, per_model), unsafe_allow_html=True)
    ...
    with tab_gradcam:
        st.markdown('<div class="gl-card">', unsafe_allow_html=True)
        st.markdown('<h3 class="gl-section-title">Attention Heatmaps</h3>', unsafe_allow_html=True)
        st.markdown(
            '<p class="gl-subtitle">'
        ...
        if cam_paths:
            ...
            for i, (name, path) in enumerate(cam_paths.items()):
                with cols[i]:
                    st.image(path, caption=f"{name}", use_container_width=True)
        else:
            st.warning("Grad-CAM generation was disabled. Enable it in Advanced options before running the assessment.")
    ...
    with tab_report:
        st.markdown('<div class="gl-card">', unsafe_allow_html=True)
        st.markdown('<h3 class="gl-section-title">Full Text Report</h3>', unsafe_allow_html=True)
        st.code(text_report, language=None)

        st.markdown('<h3 class="gl-section-title">Export Options</h3>', unsafe_allow_html=True)
